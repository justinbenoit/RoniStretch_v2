package com.ronistretch.ui 
{
	...
	public class NavigationButton extends MovieClip
	{
		...
		private var _rolloverSoundURL:String;
		private var _clickSoundURL:String;
		...
		public function NavigationButton(text:String = "", number:int=0, link:String=null, colorNormal:uint = 0xFFFFFF, colorRollover:uint = 0x999999, colorSelected:uint = 0xFF0000, subnav:Boolean = false, sidenav:Boolean = false, yPosition:int = 0) 
		{
			...
			//trace("--- NavigationButton", number, yPosition);
		...
		private function btnHandler(e:MouseEvent):void 
		{
			if (e.type == MouseEvent.ROLL_OVER) 
			{
				...
				//if (_rolloverSoundURL)
				if (Main.sndRollover2 && _selected == false)
				{
					Main.soundManager.playSound(Main.sndRollover2);
				}
				//if (this._subnav == true)
				//{
					//if (Main.sndRollover1)
					//{
						//Main.soundManager.playSoundURL(_rolloverSoundURL);
						//Main.soundManager.playSound(Main.sndRollover1);
					//}
				//}else
				//{
					//if (Main.sndRollover2)
					//{
						//Main.soundManager.playSoundURL(_rolloverSoundURL);
						//Main.soundManager.playSound(Main.sndRollover2);
					//}
			...
			}else if(e.type == MouseEvent.CLICK)	
			{
				...
				}else
				{
					...
					selected = false;		
					//if (_clickSoundURL)
					if (Main.sndClick && _selected == false)
					{
						//Main.soundManager.playSoundURL(_clickSoundURL);
						Main.soundManager.playSound(Main.sndClick);
					}
				}
		...
		public function get rolloverSoundURL():String { return _rolloverSoundURL; }
		
		public function set rolloverSoundURL(value:String):void 
		{
			_rolloverSoundURL = value;
		}
		
		public function get clickSoundURL():String { return _clickSoundURL; }
		
		public function set clickSoundURL(value:String):void 
		{
			_clickSoundURL = value;
		}
